...
if(! class_exists('WooVina_WooCommerce_Config')) {
	...
	class WooVina_WooCommerce_Config {
		...
		function add_custom_compare_button() {
			if ( is_admin() || ! function_exists( 'YITH_Woocommerce_Compare_Frontend' ) ) {
				return '';
			}

			$frontend = YITH_Woocommerce_Compare_Frontend();

			if ( ! method_exists( $frontend, 'view_table_url' ) ) {
				return '';
			}

			$link  = esc_url( $frontend->view_table_url() );
			$count = isset( $frontend->products_list ) ? count( $frontend->products_list ) : 0;

			return '<a href="' . $link . '" class="woovina-compare-header-icon yith-woocompare-open"><span class="cp-count">' . intval( $count ) . '</span></a>';
		}
	}
}
